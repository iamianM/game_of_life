...
contract CanvasHashedCopy {
    ...
    function getHash(Node memory node) public view returns(bytes memory) {
        return abi.encodePacked(bytes28(0), node.k, node.n, node.a, node.b, node.c, node.d);
    }

    function getUnhash(bytes memory b) public view returns(Node memory) {
        (bytes32 temp, uint8 k, uint8 n, bytes memory hashA, bytes memory hashB, bytes memory hashC, bytes memory hashD) = abi.decode(b, (bytes32, uint8, uint8, bytes, bytes, bytes, bytes));
    ...
    function activateNode(Node memory node, uint[] memory segments, uint j) public returns(Node memory) {
        ...
        if (segment == 0) {
            ...
            if (segments.length - 1 == j) {
                uint8 n = nodeA.n + nodeB.n + nodeD.n + 1;
            ...
            if (segments.length - 1 == j) {
                uint8 n = nodeA.n + nodeB.n + nodeC.n + 1;
